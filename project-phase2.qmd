...
# extraire la 4e ligne comme futur nom de colonnes
new_names <- as.character(rd_exp_country[3, ])

# appliquer les noms
colnames(rd_exp_country) <- new_names

# supprimer les lignes inutiles
rd_exp_country <- rd_exp_country[-3, ]
rd_exp_country <- rd_exp_country[-2, ]
rd_exp_country <- rd_exp_country[-1, ]
...
rd_long <- rd_exp_country |>
  pivot_longer(
    cols = `2000`:`2024`,     # adapter à ta dernière année
    names_to = "year",
    values_to = "value"
  )
rd_long <- rd_long |>
...
df <- df |>
  left_join(
    rd_long |> select(`Country Code`, year, value),
