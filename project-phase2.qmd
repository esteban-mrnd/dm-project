...
summarize_data <- function(df) {
  df |> summarize(
    ...
    `Missing observed values `  = sum(is.na(.data[["OBS_VALUE"]])),
    start_year = min(.data[["TIME_PERIOD"]], na.rm = TRUE),
    end_year   = max(.data[["TIME_PERIOD"]], na.rm = TRUE)
  )
}

knitr::kable(summarize_data(patents),        caption = "Patents dataset summary")
knitr::kable(summarize_data(gdp_regions),    caption = "GDP by regions dataset summary")
knitr::kable(summarize_data(precipitation),  caption = "Precipitation dataset summary")
knitr::kable(summarize_data(health_risks),   caption = "Health risks dataset summary")
knitr::kable(summarize_data(heat_stress),    caption = "Heat stress dataset summary")
knitr::kable(summarize_data(wildfires),      caption = "Wildfires dataset summary")
knitr::kable(summarize_data(rd_long),        caption = "R&D dataset summary")
...
df <- df |>
  left_join(
    rd_long |> select(`Country Code`, TIME_PERIOD, OBS_VALUE),
...
Another possible option is to use a histogram combined with a rug plot, which allows us to visualise the overall distribution of green_share while also displaying each individual observation independently of its region.

```{r}
ggplot(df, aes(x = green_share)) +
...
https://fr.euronews.com/my-europe/2025/08/28/feux-de-foret-une-annee-noire-pour-leurope?utm.com https://wmo.int/news/media-centre/wmo-confirms-2024-warmest-year-record-about-155degc-above-pre-industrial-level
